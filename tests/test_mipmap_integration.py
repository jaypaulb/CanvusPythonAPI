...
async def test_image_mipmap_integration():
    ...
    async with TestClient(config) as client:
        try:
            ...
            if not asset_hash:
                print("  📋 No existing images with valid hashes found, uploading new image...")
                ...
                if not image.hash or not image.hash.strip():
                    ...
                    for attempt in range(15):  # Try up to 15 times (30 seconds total)
                        ...
                        try:
                            updated_image = await client.client.get_image(canvas_id, image.id)
                            if updated_image.hash and updated_image.hash.strip():
                                asset_hash = updated_image.hash
                                print(f"  ✅ Hash generated after {attempt + 1} attempts: {asset_hash}")
                                break
                            else:
                                print(f"  📋 Attempt {attempt + 1}: Hash still empty")
                        except Exception as e:
                            print(f"  📋 Attempt {attempt + 1}: Error getting image: {e}")
            ...
            try:
                ...
                if isinstance(mipmap_info, dict):
                    print(f"  ✅ Mipmap info is a dictionary with keys: {list(mipmap_info.keys())}")
                ...
                if "Not supported" in error_msg:
                    print("  📝 Note: This server may not support mipmap functionality for images")
                elif "No document with id" in error_msg:
                    print("  📝 Note: Mipmap may not be generated yet or hash format may be incorrect")
            ...
            try:
                ...
                if len(level_image) > 0:
                    print(f"  ✅ Level 0 image contains {len(level_image)} bytes of data")
                    # Check if it looks like WebP data (should start with RIFF)
                    if level_image.startswith(b'RIFF'):
                        print(f"  ✅ Level 0 image appears to be valid WebP format")
                    else:
                        print(f"  ⚠️ Level 0 image doesn't appear to be WebP format")
                else:
                    print(f"  ⚠️ Level 0 image is empty")
                    
            except Exception as e:
                error_msg = str(e)
                print(f"  ❌ Error getting mipmap level image: {error_msg}")
                if "Not supported" in error_msg:
                    print("  📝 Note: This server may not support mipmap level images for images")
                elif "No document with id" in error_msg:
                    print("  📝 Note: Mipmap may not be generated yet or hash format may be incorrect")
                elif "404" in error_msg or "not found" in error_msg.lower():
                    print("  ❌ Asset not found - unexpected for valid hash")
            ...
            if 'image' in locals() and hasattr(image, 'id'):
                print(f"  📋 Cleaning up uploaded image...")
                try:
...
async def test_pdf_mipmap_integration():
    ...
    async with TestClient(config) as client:
        try:
            ...
            if not asset_hash:
                print("  📋 No existing PDFs with valid hashes found, uploading new PDF...")
            ...
            try:
                mipmap_info = await client.client.get_mipmap_info(asset_hash, page=0, canvas_id=canvas_id)
                ...
                if isinstance(mipmap_info, dict):
                    ...
                    if 'pages' in mipmap_info:
                        ...
                        if mipmap_info['pages'] > 1:
                            ...
                            try:
                                page1_info = await client.client.get_mipmap_info(asset_hash, page=1, canvas_id=canvas_id)
                                print(f"  ✅ Retrieved mipmap info for page 1: {page1_info}")
                            except Exception as e:
                ...
                print(f"  ❌ Error getting mipmap info: {error_msg}")
                if "Not supported" in error_msg:
                    print("  📝 Note: This server may not support mipmap functionality for PDFs")
                elif "No document with id" in error_msg:
                    print("  📝 Note: Mipmap may not be generated yet or hash format may be incorrect")
                elif "404" in error_msg or "not found" in error_msg.lower():
                    print("  ❌ Asset not found - unexpected for valid hash")
            ...
            try:
                level_image = await client.client.get_mipmap_level_image(asset_hash, 0, page=0, canvas_id=canvas_id)
                print(f"  ✅ Retrieved level 0 image for page 0: {len(level_image)} bytes")
                ...
                print(f"  ❌ Error getting mipmap level image: {error_msg}")
                if "Not supported" in error_msg:
                    print("  📝 Note: This server may not support mipmap level images for PDFs")
                elif "No document with id" in error_msg:
                    print("  📝 Note: Mipmap may not be generated yet or hash format may be incorrect")
                elif "404" in error_msg or "not found" in error_msg.lower():
                    print("  ❌ Asset not found - unexpected for valid hash")
            ...
            if 'pdf' in locals() and hasattr(pdf, 'id'):
                print(f"  📋 Cleaning up uploaded PDF...")
                try:
    ...
    async with TestClient(config) as client:
        try:
            ...
            try:
                ...
                print(f"  ❌ Expected 404 error but got success")
                raise Exception("Expected 404 error for invalid hash")
            except Exception as e:
                error_msg = str(e)
                if "404" in error_msg or "not found" in error_msg.lower():
                    print("  ✅ Expected 404 error for invalid hash - error handling working correctly")
                else:
