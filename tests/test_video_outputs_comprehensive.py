        ...
        try:
            ...
            for i, output in enumerate(video_outputs):
                ...
                try:
                    ...
                    suspend_result = await client.set_video_output_source(client_id, output_index, suspend_payload)
                    print(f"    ✅ Suspend successful: {suspend_result.get('suspended')}")
                ...
                try:
                    ...
                    unsuspend_result = await client.set_video_output_source(client_id, output_index, unsuspend_payload)
                    print(f"    ✅ Unsuspend successful: {unsuspend_result.get('suspended')}")
                except Exception as e:
            ...
            try:
                widgets_response = await client._request("GET", f"canvases/{canvas_id}/widgets")
                print(f"Found {len(widgets_response)} widgets on canvas")
                
                # Look for VideoOutputAnchor widgets
                vo_anchors = [w for w in widgets_response if w.get('widget_type') == 'VideoOutputAnchor']
                ...
                if vo_anchors:
                    ...
                    new_location = {"x": original_location.get('x', 0) + 100, "y": original_location.get('y', 0) + 100}
                    new_size = {"width": original_size.get('width', 800) + 50, "height": original_size.get('height', 600) + 50}
                    ...
                    try:
                        ...
                        updated_widget = await client.update_widget(canvas_id, anchor_id, move_payload)
                        ...
                        if video_outputs:
                            ...
                            print(f"Setting video output {output_index} source to VideoOutputAnchor: {anchor_id}")
                            
                            try:
                                source_result = await client.set_video_output_source(client_id, output_index, source_payload)
                                print(f"✅ Successfully set video output source to VideoOutputAnchor!")
                                print(f"Result: {json.dumps(source_result, indent=2)}")
                            except CanvusAPIError as e:
                                print(f"❌ Failed to set source to VideoOutputAnchor: {e}")
                                print("ℹ️  This is expected due to the known bug mentioned in documentation")
                            except Exception as e:
                        ...
                        try:
                            restored_widget = await client.update_widget(canvas_id, anchor_id, restore_payload)
                            print(f"✅ Successfully restored VideoOutputAnchor to original position")
                    ...
                    print("ℹ️  This is expected - VideoOutputAnchor widgets need to be created via UI")
                    print("ℹ️  The documentation mentions they cannot be created via API due to a known bug")
            ...
            if vo_anchors:
                ...
                print("❌ Setting video output source to VideoOutputAnchor - Expected to fail (known bug)")
            else:
                print("⚠️  VideoOutputAnchor widgets - Not available (need to be created via UI)")
