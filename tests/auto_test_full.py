...
from canvus_api.client import CanvusClient, CanvusAPIError
    ...
    if session.user_id:
        try:
            ...
            try:
                ...
                if user_info.admin:
                    print_error(f"{get_timestamp()} SAFETY ALERT: Cleanup aborted - Test user has admin privileges")
                    return
            except Exception as e:
                print_error(f"{get_timestamp()} Failed to verify user privileges, aborting cleanup: {e}")
                return
            
            # Delete ONLY canvases created during this test session
            print_info(f"{get_timestamp()} Cleaning up {len(session.created_canvas_ids)} test canvases")
            for canvas_id in session.created_canvas_ids:
                try:
                    ...
                    if canvas.owner_id != session.user_id:
                        print_warning(f"{get_timestamp()} Skipping canvas {canvas_id} - not owned by test user")
                        continue
                        
                    await client.delete_canvas(canvas_id)
                    print_success(f"{get_timestamp()} Deleted test canvas: {canvas_id}")
                except Exception as e:
                    print_warning(f"{get_timestamp()} Failed to delete test canvas {canvas_id}: {e}")
            
            # Delete test token
            if session.token_id:
                try:
                    await client.delete_token(session.user_id, session.token_id)
                    print_success(f"{get_timestamp()} Deleted test token: {session.token_id}")
                except Exception as e:
...
async def ensure_test_user_cleanup(client: CanvusClient, email: str) -> None:
    ...
    try:
        ...
        if user_id:
            ...
            print_info(f"{get_timestamp()} Found existing test user (ID: {user_id}), cleaning up...")
...
async def create_test_user(client: CanvusClient, user_data: Dict[str, Any]) -> Tuple[int, str, str]:
    ...
    if user_info.admin:
        raise ValueError("Safety check failed: Test user was created with admin privileges")
    ...
    try:
        ...
        async with CanvusClient(
            ...
            try:
                ...
                user_id, token_id, plain_token = await create_test_user(client, config["test_user"])
                session.user_id = user_id
                ...
                async with CanvusClient(
                    ...
                    try:
                        ...
                        if admin_client_id:
                            await test_workspace_operations(test_client, admin_client_id)
