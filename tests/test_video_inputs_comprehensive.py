...
async def test_video_inputs_comprehensive():
    ...
    async with CanvusClient(
        ...
        try:
            ...
            print(f"Found {len(existing_video_inputs)} existing video input widget(s) on canvas")
            
            for i, widget in enumerate(existing_video_inputs):
                print(f"Widget {i}: {widget.get('id')} - {widget.get('name', 'Unnamed')}")
            ...
            if video_input_sources:
                ...
                for i, source in enumerate(video_input_sources):
                    ...
                    try:
                        ...
                        new_widget = await client.create_video_input(canvas_id, create_payload)
                        ...
                        verification_widgets = await client.list_canvas_video_inputs(canvas_id)
                        found_widget = None
                        ...
                        if found_widget:
                            ...
                            print(f"  📍 Verified Location: {found_widget.get('location')}")
                            print(f"  📍 Verified Size: {found_widget.get('size')}")
                            print(f"  📍 Verified State: {found_widget.get('state')}")
                        else:
                            print(f"  ❌ GET Verification: Widget NOT found in canvas list!")
                            print(f"  📊 Total widgets in canvas: {len(verification_widgets)}")
                ...
                print(f"Found {len(updated_video_inputs)} total video input widget(s) on canvas")
                ...
                for widget in updated_video_inputs:
                    if widget.get('id') in test_widget_ids:
                        ...
                        print(f"✅ Found widget: {widget.get('id')} at {widget.get('location')}")
                
                print(f"✅ {len(found_widgets)}/{len(test_widget_ids)} new widgets found in canvas list")
            ...
            if test_widget_ids:
                ...
                print(f"⏰ Waiting 5 seconds before deletion so you can validate the {len(test_widget_ids)} widgets...")
                print(f"Widget IDs: {test_widget_ids}")
                print(f"Locations: Grid starting at (600, 600) with 800px horizontal spacing, 600px vertical spacing")
                ...
                for widget_id in test_widget_ids:
                    try:
                        ...
                        print(f"✅ Successfully deleted test video input widget: {widget_id}")
                        deleted_count += 1
                    except CanvusAPIError as e:
                        print(f"❌ Failed to delete video input widget {widget_id}: {e}")
                    except Exception as e:
                        print(f"❌ Unexpected error deleting video input widget {widget_id}: {e}")
                ...
                print(f"Found {len(final_video_inputs)} video input widget(s) after deletion")
                
                # Check if any of our widgets still exist
                remaining_widgets = [w.get('id') for w in final_video_inputs if w.get('id') in test_widget_ids]
                if not remaining_widgets:
                    print("✅ All test widgets successfully removed from canvas")
                else:
                    print(f"❌ {len(remaining_widgets)} test widgets still exist: {remaining_widgets}")
            ...
            if video_input_sources:
                ...
                print("⚠️  Video input widget creation - Not tested (no sources available)")
