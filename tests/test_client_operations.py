"""
Test suite for client operations.
"""
...
from canvus_api.models import Workspace
...
async def wait_for_workspace_canvas(
    ...
    for i in range(max_retries):
        ...
        print_info(f"Current workspace state: canvas_id={workspace.canvas_id}, server_id={workspace.server_id}")
...
async def test_workspace_operations(client: CanvusClient, client_id: str) -> None:
    ...
    try:
        ...
        print_info(f"Testing with workspace {workspace.index}: {workspace.workspace_name}")
        ...
        for canvas in canvases:
            if canvas.name.startswith("Workspace Test Canvas"):
                try:
                    ...
                    print_warning(f"Failed to clean up existing test canvas {canvas.id}: {e}")
        ...
        try:
            ...
            print_info(f'Opening canvas with payload: {{"canvas_id": "{test_canvas.id}", "server_id": "{workspace.server_id}"}}')
            await client._request(
                "POST",
            ...
            if not await wait_for_workspace_canvas(client, client_id, workspace.index, test_canvas.id):
            ...
            print_info(f"Original settings: pinned={original_settings['pinned']}, info_panel_visible={original_settings['info_panel_visible']}")
            print_info(f"Original view rectangle: x={original_settings['view_rectangle']['x']} y={original_settings['view_rectangle']['y']} width={original_settings['view_rectangle']['width']} height={original_settings['view_rectangle']['height']}")
            ...
            updated = await client.update_workspace(client_id, workspace.index, settings_update)
            ...
            updated = await client.update_workspace(client_id, workspace.index, view_update)
            
            # Verify view rectangle changed
            if (updated.view_rectangle.x == original_settings["view_rectangle"]["x"] and
                updated.view_rectangle.y == original_settings["view_rectangle"]["y"] and
                updated.view_rectangle.width == original_settings["view_rectangle"]["width"] and
                updated.view_rectangle.height == original_settings["view_rectangle"]["height"]):
                print_error("View rectangle did not change")
                return
            print_success("Updated workspace view rectangle")
            
            # 7. Restore original settings
            restored = await client.update_workspace(client_id, workspace.index, {
            ...
            if restored.info_panel_visible != original_settings["info_panel_visible"]:
                ...
                return
            if (restored.view_rectangle.x != original_settings["view_rectangle"]["x"] or
                restored.view_rectangle.y != original_settings["view_rectangle"]["y"] or
                restored.view_rectangle.width != original_settings["view_rectangle"]["width"] or
                restored.view_rectangle.height != original_settings["view_rectangle"]["height"]):
            ...
            print_info(f'Restoring canvas with payload: {{"canvas_id": "{original_canvas_id}", "server_id": "{workspace.server_id}"}}')
            await client._request(
                "POST",
                f"clients/{client_id}/workspaces/{workspace.index}/open-canvas",
            ...
            if not await wait_for_workspace_canvas(client, client_id, workspace.index, original_canvas_id):
