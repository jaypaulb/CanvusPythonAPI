...
class TestDataManager:
            ...
            if "already in use" in str(e):
                ...
                for user in users:
                    if user.email == self.config.get_test_data("test_user")["email"]:
                        self.config.set_test_data("test_user", user.model_dump() if hasattr(user, 'model_dump') else dict(user))
                        break
            else:
                raise
    ...
    async def _create_test_folder(self) -> Dict[str, Any]:
        ...
        folder_dict = folder.model_dump() if hasattr(folder, 'model_dump') else dict(folder)
...
class TestClient:
    ...
    def __init__(self, config: TestConfig):
        ...
        self.client = CanvusClient(config.server_url, config.api_key, verify_ssl=config.verify_ssl)
    ...
    async def authenticate(self, use_admin: bool = True) -> None:
        ...
        credentials = self.config.admin_credentials if use_admin else self.config.user_credentials
    ...
    def get_test_canvas_id(self) -> str:
        ...
        if not canvas_id:
            raise ValueError("Test canvas not found. Run setup_test_environment() first.")
        return canvas_id
    ...
    def get_test_folder_id(self) -> str:
        ...
        if not folder_id:
            raise ValueError("Test folder not found. Run setup_test_environment() first.")
        return folder_id
    ...
    def get_test_group_id(self) -> str:
        ...
        if not group_id:
            raise ValueError("Test group not found. Run setup_test_environment() first.")
        return group_id
