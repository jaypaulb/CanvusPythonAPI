        ...
        try:
            ...
            try:
                ...
                for i, workspace in enumerate(workspaces):
                    print(f"Workspace {i}: {workspace.workspace_name} (user: {workspace.user})")
            ...
            print(f"Initial state - suspended: {initial_suspended}, source: '{initial_source}'")
            ...
            try:
                ...
                if verify_suspended == new_suspended:
                    ...
                    print(f"❌ Suspend functionality failed - expected {new_suspended}, got {verify_suspended}")
            ...
            if workspaces:
                ...
                try:
                    ...
                    if verify_source == workspace_source:
                        ...
                        print(f"⚠️  Source setting may have failed - expected '{workspace_source}', got '{verify_source}'")
                ...
                try:
                    clear_result = await client.set_video_output_source(client_id, output_index, clear_payload)
                    ...
                    if verify_source_cleared == "":
                        ...
                        print(f"⚠️  Source clearing may have failed - expected empty string, got '{verify_source_cleared}'")
                        
                except CanvusAPIError as e:
                    print(f"⚠️  Source clearing failed with API error: {e}")
                    print("ℹ️  This may be due to permission restrictions or server limitations")
            else:
            ...
            print(f"Restoring to original state - suspended: {initial_suspended}, source: '{initial_source}'")
            
            try:
                restore_result = await client.set_video_output_source(client_id, output_index, restore_payload)
                ...
                if final_suspended == initial_suspended and final_source == initial_source:
                    print("✅ Successfully restored to original state")
                else:
                    print(f"⚠️  Restoration may have failed - suspended: {final_suspended} vs {initial_suspended}, source: '{final_source}' vs '{initial_source}'")
            ...
            # Test invalid index
            try:
                await client.set_video_output_source(client_id, 999, {"suspended": True})
            ...
            try:
                await client.set_video_output_source(client_id, output_index, {"invalid_field": "value"})
