...
class ManualVisualParentIDTest:
    ...
    async def save_results(self):
        ...
        # Save as CSV
        csv_filename = (
            f"manual_visual_test_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
        )
        if self.test_data:
    ...
    async def create_analysis_summary(self):
        ...
        summary_filename = (
            f"manual_analysis_summary_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
        )
        ...
        with open(summary_filename, "w") as f:
            ...
            f.write(
                "- **NOTE1_TO_NOTE2 screenshot** - After RED note reparented to GREEN\n"
            )
            f.write(
                "- **NOTE2_TO_NOTE3 screenshot** - After GREEN note reparented to BLUE\n\n"
            )
