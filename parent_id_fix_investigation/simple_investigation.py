...
class SimpleParentIDInvestigator:
    ...
    async def setup_test_environment(self):
        ...
        for i, parent_info in enumerate(parent_locations):
            parent = await self.client.create_note(self.canvas.id, {
        ...
        if "Parent A" in self.parent_widgets:
            nested_parent = await self.client.create_note(self.canvas.id, {
    ...
    async def create_test_notes(self):
        ...
        for note_info in test_locations:
            note = await self.client.create_note(self.canvas.id, {
    ...
    async def change_parent_and_record(self, widget_id: str, new_parent_id: Optional[str], test_id: str):
        ...
        # Change parent_id
        updated_widget = await self.client.update_note(self.canvas.id, widget_id, {
