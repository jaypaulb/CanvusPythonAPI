...
class ManualVisualRelativeCoordTest:
    ...
    async def create_test_notes(self):
        ...
        print(
            f"✅ Created RED Note 1: {note1_data.id} at [{note1_x}, {note1_y}] (600x600)"
        )
        ...
        print(
            f"✅ Created GREEN Note 2: {note2_data.id} at [{note2_x}, {note2_y}] (600x600)"
        )
        ...
        print(
            f"✅ Created BLUE Note 3: {note3_data.id} at [{note3_x}, {note3_y}] (600x600)"
        )
    ...
    async def record_widget_state(self, stage: str, widgets: List[Dict[str, Any]]):
        ...
        for widget in widgets:
            if (
                widget.get("widget_type") == "Note"
                and widget.get("id") in self.note_ids
            ):
    ...
    async def change_parent_id_with_discovered_formula(
        self, widget_id: str, new_parent_id: str
    ):
        """Change the parent ID of a widget using the discovered formula: -(width) - location - 30"""
        print(
            f"🔄 Changing parent of {widget_id} to {new_parent_id} using discovered formula"
        )
        ...
        try:
            ...
            print(
                f"✅ Successfully applied formula-based offset: [{calculated_offset_x}, {calculated_offset_y}]"
            )
    ...
    async def run_test_sequence(self):
        ...
        await self.change_parent_id_with_discovered_formula(
            self.note_ids[0], self.note_ids[1]
        )
        ...
        await self.change_parent_id_with_discovered_formula(
            self.note_ids[1], self.note_ids[2]
        )
        ...
        await self.change_parent_id_with_discovered_formula(
            self.note_ids[2], self.note_ids[0]
        )
    ...
    async def create_analysis_summary(self):
        ...
        with open(summary_filename, "w") as f:
            ...
            f.write(
                f"**Test Type:** Manual snapshot analysis with relative coordinate adjustment\n\n"
            )
